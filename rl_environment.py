...
HOUSE_MULT = 5             # reward multiplier on each house build step
HOTEL_MULT = 4             # reward multiplier on each hotel-equivalent step
...
# Ordered positions of all 28 buyable squares (used as a fixed property index)
BUYABLE_POSITIONS: list[int] = [sq["pos"] for sq in BOARD
                                 if sq["type"] in ("property", "railroad", "utility")]

# Color-group index for each buyable position (used in state vector)
_COLOR_GROUP_IDX = {
    "brown": 0, "light_blue": 1, "pink": 2, "orange": 3,
    "red": 4, "yellow": 5, "green": 6, "dark_blue": 7,
}
_PROP_GROUP: dict[int, int] = {}
for _sq in BOARD:
    if _sq["type"] == "property":
        _PROP_GROUP[_sq["pos"]] = _COLOR_GROUP_IDX[_sq["color"]]
    elif _sq["type"] == "railroad":
        _PROP_GROUP[_sq["pos"]] = 8
    elif _sq["type"] == "utility":
        _PROP_GROUP[_sq["pos"]] = 9
...
# ---------------------------------------------------------------------------
def extract_state(player: Player, bank: Bank, all_players: list["Player"],
                  board: list["Location"]) -> dict:
    """
    Returns a state dict with three sections:
      - 10 own-player aggregate features
      - 4 × 3 opponent aggregate features
      - 12 × 28 per-property features (one row per buyable square, fixed order)

    Total flat state dimension: 10 + 12 + 336 = 358.
    """
    opponents = [p for p in all_players if p.pid != player.pid]
    pos_map   = {loc.pos: loc for loc in board}

    # ── own-player aggregate ──────────────────────────────────────────────
    own = {
        "cash":           player.cash,
    ...
    opp_feats = [
        {
            "cash":           opp.cash,
            "net_worth":      opp.net_worth(),
            "num_monopolies": len(opp.monopolies),
            "in_jail":        int(opp.in_jail),
        }
        for opp in opponents
    ]

    # ── per-property features (28 properties, fixed BUYABLE_POSITIONS order) ──
    # 12 features each:
    #   owned_by_agent, owned_by_opp[0-2], unowned,
    #   is_mortgaged, house_count, current_rent, mortgage_value,
    #   house_cost, full_color_group, color_group_id
    properties = []
    for pos in BUYABLE_POSITIONS:
        loc   = pos_map[pos]
        owner = loc.owned_by

        # ownership (5 flags; opponent slots padded to always be 3)
        owned_by_opps = [float(owner is opp) for opp in opponents]
        while len(owned_by_opps) < 3:
            owned_by_opps.append(0.0)

        # structural
        h_count  = (loc.num_houses + loc.num_hotels * 5) / 5.0  # hotel = 5 houses

        # estimated current rent (normalised by Boardwalk hotel: 2000)
        if loc.is_mortgaged or owner is None:
            rent_n = 0.0
        elif loc.type == "railroad":
            rent_n = RAILROAD_RENTS[min(owner.num_railroads - 1, 3)] / 2000.0
        elif loc.type == "utility":
            mult   = UTILITY_MULTIPLIERS[0 if owner.num_utilities == 1 else 1]
            rent_n = (7 * mult) / 2000.0          # expected dice ≈ 7
        else:
            if loc.num_hotels > 0:
                rent_n = loc.rents[5] / 2000.0
            elif loc.num_houses > 0:
                rent_n = loc.rents[loc.num_houses] / 2000.0
            elif loc.color in owner.monopolies:
                rent_n = (loc.rents[0] * 2) / 2000.0
            else:
                rent_n = loc.rents[0] / 2000.0

        # does the current owner hold a complete group for this property?
        if owner is not None:
            if loc.type == "property":
                full_group = float(loc.color in owner.monopolies)
            elif loc.type == "railroad":
                full_group = float(owner.num_railroads == 4)
            else:  # utility
                full_group = float(owner.num_utilities == 2)
        else:
            full_group = 0.0

        properties.append({
            "owned_by_agent": float(owner is player),
            "owned_by_opps":  owned_by_opps,      # list[float], length 3
            "unowned":        float(owner is None),
            "is_mortgaged":   float(loc.is_mortgaged),
            "house_count":    h_count,
            "current_rent":   rent_n,
            "mortgage_value": loc.mortgage_value / 200.0,
            "house_cost":     loc.house_cost / 200.0,
            "full_group":     full_group,
            "color_group_id": _PROP_GROUP[pos] / 9.0,
        })

    return {**own, "opponents": opp_feats, "properties": properties}


def state_vector(state: dict) -> list[float]:
    """
    Flat numeric feature list for the policy network.
    Length = 10 (own) + 12 (3 opponents × 4) + 336 (28 properties × 12) = 358.
    """
    # own-player (10)
    vec = [
        state["cash"] / 1000.0,
    ...
    for opp in state["opponents"]:
        vec.extend([
            opp["cash"] / 1000.0,
            opp["net_worth"] / 5000.0,
            opp["num_monopolies"] / 8.0,
            float(opp["in_jail"]),
        ])
    # per-property (12 × 28 = 336)
    for prop in state["properties"]:
        vec.append(prop["owned_by_agent"])
        vec.extend(prop["owned_by_opps"])   # 3 values
        vec.append(prop["unowned"])
        vec.append(prop["is_mortgaged"])
        vec.append(prop["house_count"])
        vec.append(prop["current_rent"])
        vec.append(prop["mortgage_value"])
        vec.append(prop["house_cost"])
        vec.append(prop["full_group"])
        vec.append(prop["color_group_id"])
    return vec
...
def enhanced_net_worth(player: "Player") -> float:
    ...
    Net worth with monopoly bonus per Bonjour et al. (2022), Eq. 2-3:
      pa = (price - mv) * b + house_val + hotel_val
    where b = 2.0 if the property completes a monopoly, else 1.5,
    mv = unmortgage_cost if mortgaged, else 0.
    Houses are valued at HOUSE_MULT * house_cost each; hotels at
    HOTEL_MULT * house_cost per step (HOUSE_HOTEL_THRESHOLD+1 steps total).
    Multipliers > 1 give an immediate positive gradient signal for improving.
    """
    ...
    for loc in player.assets:
        ...
        house_val = loc.num_houses * loc.house_cost * HOUSE_MULT
        hotel_val = loc.num_hotels * (HOUSE_HOTEL_THRESHOLD + 1) * loc.house_cost * HOTEL_MULT
        pa = (loc.price - mv) * b + house_val + hotel_val
        nw += pa
    return nw
...
class AggressiveBuyer(Agent):
    ...
    def decide_mortgage(self, player, candidates, state):
        if not candidates or player.cash >= GO_REWARD:
            return None
...
class ConservativeBuyer(Agent):
    ...
    def decide_mortgage(self, player, candidates, state):
        if not candidates or player.cash >= self.CASH_BUFFER:
            return None
...
class MonopolyEnv:
    ...
    def reset(self) -> dict:
        ...
        return extract_state(self.current_player(), self.bank, self.players, self.board)
    ...
    def step(self) -> tuple[dict, float, bool, dict]:
        ...
        if self.done:
            ...
            return extract_state(p, self.bank, self.players, self.board), 0.0, True, {}
        ...
        obs  = extract_state(player, self.bank, self.players, self.board)
    ...
    def _play_turn(self, player: Player) -> float:
        ...
        # --- purchasable square ---
            ...
            if square.owned_by is None:
                # buy decision
                state = extract_state(player, self.bank, self.players, self.board)
...
if __name__ == "__main__":
    ...
    print(f"State vector length: {len(state_vector(extract_state(players[0], env.bank, players, env.board)))}")
