...
class MonopolyEnv:
    ...
    def step(self) -> tuple[dict, float, bool, dict]:
        ...
        if len(active) <= 1:
            ...
            return extract_state(p, self.bank), 1.0, True, {}
    ...
    def _play_turn(self, player: Player) -> float:
        ...
        if passed_go:
            ...
            reward        += 0.1  # small reward for collecting GO salary
        ...
        if square.type == "tax":
            ...
            self.bank.cash += tax
            if player.cash < 0:
                reward += self._handle_bankruptcy(player, None, abs(player.cash))
        ...
        # --- purchasable square ---
            ...
            if square.owned_by is None:
                ...
                if player.agent.decide_buy(player, square, state):
                    if _apply_buy(player, square, self.bank):
                        reward += 0.05
                ...
                if player.cash < 0:
                    reward += self._handle_bankruptcy(player, square.owned_by, rent)
