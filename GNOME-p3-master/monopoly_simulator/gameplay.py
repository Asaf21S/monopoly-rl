...
from monopoly_simulator import simple_agents
...
def simulate_game_instance(game_elements, history_log_file=None, np_seed=2, enable_summary=False):
    """
    ...
    :param enable_summary: If True, print a human-readable end-game summary table.
    :return: None
    """
    ...
    while num_active_players > 1:
        ...
        if current_player.current_cash < 0:
            ...
            if code == flag_config_dict['failure_code'] or current_player.current_cash < 0:
                ...
                elimination_order.append(current_player.player_name)
    ...
    if enable_summary:
        diagnostics.print_game_summary(game_elements, elimination_order, turn_count=num_die_rolls)
...
def _build_agent(agent_type):
    """Return an Agent for the given type string, or the background agent if None/'background'."""
    if agent_type is None or agent_type == 'background':
        return Agent(**background_agent_v3_1.decision_agent_methods)
    return simple_agents.make_agent(agent_type)


def play_game(agent_types=None, enable_summary=False):
    """
    ...
    :param agent_types: Optional dict mapping player names to strategy strings
        ('always_buy', 'never_buy', 'cautious', 'aggressive', 'background').
        Players not listed default to the background agent.
    :param enable_summary: If True, print a human-readable end-game summary table.
    ...
    for p in ['player_1', 'player_2', 'player_3', 'player_4']:
        player_decision_agents[p] = _build_agent(agent_types.get(p))
    ...
    if player_decision_agents['player_1'].startup(game_elements) == flag_config_dict['failure_code'] or \
        ...
        winner = simulate_game_instance(game_elements, enable_summary=enable_summary)
...
def play_game_in_tournament(game_seed, novelty_info=False, inject_novelty_function=None,
                            agent_types=None, enable_summary=False):
    ...
    if not novelty_info:
        if player_decision_agents['player_1'].startup(game_elements) == flag_config_dict['failure_code'] or \
            ...
            logger.debug("Sucessfully initialized all player agents.")
            winner = simulate_game_instance(game_elements, history_log_file=None, np_seed=game_seed, enable_summary=enable_summary)
            if player_decision_agents['player_1'].shutdown() == flag_config_dict['failure_code'] or \
                    player_decision_agents['player_2'].shutdown() == flag_config_dict['failure_code'] or \
                    player_decision_agents['player_3'].shutdown() == flag_config_dict['failure_code'] or \
                    player_decision_agents['player_4'].shutdown() == flag_config_dict['failure_code']:
